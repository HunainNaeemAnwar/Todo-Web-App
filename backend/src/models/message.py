...
class Message(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    user_id: str = Field(
        max_length=64, index=True, description="Owner of the message (user isolation)"
    )

    conversation_id: str = Field(
        max_length=64, index=True, description="Parent conversation identifier"
    )

    role: str = Field(max_length=20, description="Message role: 'user' or 'assistant'")

    content: str = Field(
        max_length=10000, description="Message content (max 10000 characters)"
    )
    ...
    def create_user_message(
        cls, user_id: str, conversation_id: str, content: str
    ) -> "Message":
